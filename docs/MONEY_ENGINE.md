...
- **Self-employment tax.** 1099 / side-gig profit (`selfEmploymentIncome`) pays income tax plus Schedule SE tax: both halves of FICA on 92.35% of profit, sharing the SS wage base and Additional Medicare threshold with W-2 wages, nothing under $400 of net earnings, and half (ex-surtax) deducted from taxable income. No QBI deduction yet — errs conservative.
- **Irregular income.** Logged one-offs (`income/irregular.ts`) reach the plan as the **trailing 365-day sum, unscaled** — not a 30-day run-rate, which would turn one payout into a phantom salary. They add to gross and take-home but not to gross-based (payroll) dials or the employer match. Untaxed entries (gifts) bypass tax. Entries marked `toBudget` skip the plan entirely; their after-tax value (tax = the difference they make on top of the plan) is added whole to that month's budget.
- **Outputs:** `taxableIncome`, `federal`, `fica`, `selfEmployment`, `state`, `total`, plus `effectiveRate` and `marginalRate` (marginal drives "what does the next dollar into 401k actually save me").
