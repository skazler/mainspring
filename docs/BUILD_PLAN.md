...
| **2.5. Positions + capital gains** | `lots` event table (holdings derived); pure valuation + realized-gain (FIFO/specific-ID); `engine/tax/capgains.ts` (LT 0/15/20 + NIIT, ST as ordinary) composed into `computeTax`; golden tests. See [`STOCK_MANAGEMENT.md`](./STOCK_MANAGEMENT.md) | a buy/sell ledger rolls up to a holding + cost basis; realized ST/LT gains and capital-gains tax are correct to the cent (prices passed in); live valuation lands with Phase 6 |
...
**Ordering note for agents:** phases 1–5 are the spine and must land in order (each depends on the prior contract). Phase 2.5 (positions + capital gains) depends only on Phase 2 and the Phase 1 schema; it does **not** block Phase 3, so it can run in parallel with 3–4 (its *live* valuation completes with Phase 6). 6–8 can parallelize across agents once 5 is stable. 9–10 are independent opt-ins.
